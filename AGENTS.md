...
1. **Repository Structure**:
    ...
    - `infra/`: Contains the Terraform infrastructure-as-code files. Note that `terraform.tfvars.example` is the source of truth for configurable Terraform input values.
    - `Dockerfile`: Used for building the Cloud Run container.
2. **Synchronize Changes**: If you make any changes to the environment variables, requirements, or Terraform variables, you **must** update the `README.md` and `infra/terraform.tfvars.example` to ensure setup instructions remain accurate.
